...
`packages/DtBlkFx-VST3-Presets.zip` contains all 43 factory programs as
standard `.vstpreset` files. Extract its `VST3 Presets` folder into
`~/Library/Audio/Presets/` or import individual files in a compatible VST3
host.

`packages/DtBlkFx-Ableton-Live-Presets.zip` contains the same 43 states as
Ableton Live 11 `.adv` device presets. Extract its `DtBlkFx` folder into the
Ableton User Library. The pack is separate because Live does not populate its
device preset field from the VST3 program-list API.
