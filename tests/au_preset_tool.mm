...
int main()
{
    ...
    AudioUnit unit {};
    if(!Check(AudioComponentInstanceNew(component, &unit) == noErr, "Audio Unit instantiation failed")
        || !Check(AudioUnitInitialize(unit) == noErr, "Audio Unit initialization failed"))
        return 1;

    CFArrayRef presets {};
    UInt32 presetBytes = sizeof(presets);
    if(!Check(AudioUnitGetProperty(
                  unit, kAudioUnitProperty_FactoryPresets, kAudioUnitScope_Global, 0, &presets, &presetBytes)
               == noErr,
           "factory preset list unavailable")
        || !Check(presets != nullptr && CFArrayGetCount(presets) == PresetCount, "factory preset list is incomplete"))
        return 1;
    ...
    for(CFIndex index = 0; index < PresetCount; ++index)
    {
        ...
        const double expected = static_cast<double>(index) / static_cast<double>(PresetCount - 1);
        if(!Check(AudioUnitGetParameter(unit, ProgramParameterId, kAudioUnitScope_Global, 0, &selected) == noErr,
