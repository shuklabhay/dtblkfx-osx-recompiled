...
int main(int argc, const char* argv[])
{
    @autoreleasepool
    {
        ...
        if(!Check(controller != nullptr, "edit controller not found")
            || !Check(
                controller->getParameterCount() == 46, "expected 44 legacy parameters, bypass, and factory programs")
        ...
        if(!Check(controller->setParamNormalized(4, changedValue) == Steinberg::kResultOk, "host parameter edit failed")
        ...
        if(!Check(![before isEqualToData:after], "editor did not redraw after a host parameter edit")
            ...
            || !Check(
                [&]
                {
                    ...
                    if(processor->setBusArrangements(&inputArrangement, 1, &outputArrangement, 1)
                            != Steinberg::kResultOk
                    ...
                    auto* startupProgramQueue
                        = startupChanges.addParameterData(DtBlkVst3::ProgramParameterId, startupQueueIndex);
                        ...
                        for(int block = 0; block < 128; ++block)
                        {
                            ...
                            for(Steinberg::int32 sample = 0; sample < setup.maxSamplesPerBlock; ++sample)
                            {
                                ...
                                if(result.firstOutput < 0
                                    ...
                                    result.firstOutput
                                        = static_cast<std::int64_t>(block) * setup.maxSamplesPerBlock + sample;
                            }
                        }
                        return result;
                    };
                    ...
                    if([after isEqualToData:firstSpectrum] || processor->setProcessing(false) != Steinberg::kResultOk
                        || processor->setProcessing(true) != Steinberg::kResultOk)
                        return false;
                    ...
                    auto* bypassOnQueue = bypassOn.addParameterData(DtBlkVst3::BypassParameterId, bypassQueueIndex);
                    if(!bypassOnQueue || bypassOnQueue->addPoint(0, 1.0, bypassPointIndex) != Steinberg::kResultTrue)
                        return false;
                    ...
                    auto* bypassOffQueue = bypassOff.addParameterData(DtBlkVst3::BypassParameterId, bypassQueueIndex);
                    if(!bypassOffQueue || bypassOffQueue->addPoint(0, 0.0, bypassPointIndex) != Steinberg::kResultTrue)
                        return false;
                    ...
                    if(postBypassMaximum > 1e-5f || processor->setProcessing(false) != Steinberg::kResultOk
                        || processor->setProcessing(true) != Steinberg::kResultOk)
                        return false;
                    ...
                    if(!programQueue || programQueue->addPoint(0, 2.0 / 42.0, pointIndex) != Steinberg::kResultTrue)
                        return false;
                    ...
                    {
                        if(![secondSpectrum writeToFile:[NSString stringWithUTF8String:screenshotPath] atomically:YES])
                            return false;
                    }
                    if(processor->setProcessing(false) != Steinberg::kResultOk)
                        return false;
        ...
        std::cout << "PASS: module, programs, ordered startup, state, audio, live FFT, bypass continuity, "
                     "disable/re-enable, NSView lifecycle\n";
        return 0;
    }
}
